...
class NeuralNetwork:
    ...
    def train(self, X, T, nIterations=100, verbose=False, weightPrecision=0, errorPrecision=0, saveWeightsHistory=False):
        ...
        def objectiveF(w):
            ...
            Y = Zprev.dot(self.W[1:,:]) + self.W[0:1,:]
        ...
        def gradF(w):
            ...
            for i in range(len(self.nhs)):
                V = self.Vs[i]
                Zprev = np.tanh(Zprev.dot(V[1:,:]) + V[0:1,:])
                Z.append(Zprev)
            Y = Zprev.dot(self.W[1:,:]) + self.W[0:1,:]
            delta = -(T - Y) / (X.shape[0] * T.shape[1])
            dW = 2 * np.vstack(( np.ones((1,delta.shape[0])).dot(delta), 
                                 Z[-1].T.dot(delta) ))
            dVs = []
            delta = (1 - Z[-1]**2) * (delta.dot(self.W[1:,:].T))
            for Zi in range(len(self.nhs), 0, -1):
                Vi = Zi - 1 # because X is first element of Z
                dV = 2 * np.vstack(( np.ones((1,delta.shape[0])).dot(delta),
                                     Z[Zi-1].T.dot(delta) ))
                dVs.insert(0,dV)
                delta = (delta.dot(self.Vs[Vi][1:,:].T)) * (1 - Z[Zi-1]**2)
